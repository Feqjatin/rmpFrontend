import { useState } from "react";
import InterviewerManage from "./InterviewerManage.jsx";
import InterviewerReschedule from "./InterviewerReschedule.jsx";

function InterviewerDashboard() {
  const [activePage, setActivePage] = useState("interview");

  const renderContent = () => {
    switch (activePage) {
      case "interview":
        return <InterviewerManage/>;
      case "reschedule":
        return <InterviewerReschedule/>;
      default:
        return <p>Select a section from the sidebar</p>;
    }
  };
  
  const handleStyle = (page) => {
    if (activePage === page) {
      return `px-4 py-2 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 transition mb-2`;
    } else {
      return `px-4 py-2 bg-gray-400 text-white rounded-lg font-medium hover:bg-blue-700 transition mb-2`;
    }
  };
  
  return (
    <div style={{ display: "flex", minHeight: "100vh" }}>
      <div style={{ width: "200px", background: "#f4f4f4", padding: "20px", minHeight: "100vh" }}>
        <h3>Interviewer Panel</h3>
        <hr />
        <br />
        <ul style={{ listStyle: "none", padding: 0 }}>
          <li
            className={handleStyle("interview")}
            onClick={() => setActivePage("interview")}
            style={{ cursor: "pointer" }}
          >
            Manage Interview
          </li>
          <li
            className={handleStyle("reschedule")}
            onClick={() => setActivePage("reschedule")}
            style={{ cursor: "pointer" }}
          >
            Reschedule Requests
          </li>
        </ul>
      </div>
      <div style={{ flex: 1, padding: "20px" }}>{renderContent()}</div>
    </div>
  );
}

export default InterviewerDashboard;